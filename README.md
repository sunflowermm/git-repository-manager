...
三栏布局：左侧仓库列表、中间工作区、右侧操作日志。

![主界面总览](docs/screenshots/main-overview.png)

### 工作区与文件变更

选中仓库后可查看远程信息、快速提交、快捷操作与变更文件列表。

![主界面 - 工作区与文件变更](docs/screenshots/main-workspace.png)

提交成功后，日志会输出真实提交信息，并附带文件数与行数统计（如 `+158 -0 行`）：

![主界面 - 提交行数统计日志](docs/screenshots/main-with-log.png)

### 平台配置

按平台配置 SSH / Token、邮箱与私钥；可选填写**应用更新代理端口**（直连失败时走本地 HTTP 代理重试）。

![平台配置](docs/screenshots/platform-config.png)

### 同步配置

配置「主仓库 → 多个从仓库」同步组，适合同一项目在 GitHub / Gitee / GitCode 多远端镜像。
...
详见 [history/](history/) 目录（如 [v1.2.6](history/v1.2.6.md)）。

截图资源位于 [docs/screenshots/](docs/screenshots/)。
